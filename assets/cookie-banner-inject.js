...
(function () {
  ...
  // Hide native banner to prevent unstyled flash
  ...
  // ===== Styles (banner ≥1550 row, <1550 stacked) + branded prefs =====
  var css = `
/* =======================
   MAIN BANNER
   ======================= */
...
/* Buttons row */
.shopify-pc__banner__btns{ gap:0;flex-direction:row-reverse;width:auto;white-space:nowrap;border-top:0; }
...
.shopify-pc__banner__dialog button.shopify-pc__banner__btn-accept{
  border:1px solid var(--_adult---adult-red) !important;border-top:none !important;
...
@media (min-width:1550px){
  .shopify-pc__banner__dialog button.shopify-pc__banner__btn-decline{ border-left:1px solid var(--_adult---adult-red) !important; }
}

/* Manage Prefs button */
.shopify-pc__banner__dialog button.shopify-pc__banner__btn-manage-prefs{
  padding-left:.5rem !important;padding-right:.5rem !important;text-decoration:none;color:var(--_adult---adult-red) !important;
}
...
/* Header + title */
.shopify-pc__prefs__dialog header{ padding:0rem !important; border-bottom:1px solid var(--_adult---adult-red) !important; }
.shopify-pc__prefs__dialog header h2{ color:var(--_adult---adult-red) !important; font-size:1em !important; margin-left:.5rem !important; }

/* Header actions */
.shopify-pc__prefs__header-actions{ padding:0 !important; height:var(--_adult---default-height) !important; margin-right:-1px !important; }
.shopify-pc__prefs__header-actions button{
  ...
  padding:.5rem .5rem !important; margin:-1px !important; font-size:100% !important; line-height:100% !important;
}
.shopify-pc__prefs__header-actions button.primary{ background:var(--_adult---adult-red) !important; color:#fff !important; }
.shopify-pc__prefs__header-actions button:focus{ outline:none !important; box-shadow:unset !important; }
.shopify-pc__prefs__header-close:focus{ outline:none !important; box-shadow:unset !important; }
.shopify-pc__prefs__header-close{ display:none !important; }
.shopify-pc__prefs__header-actions button:hover{ cursor:pointer; text-underline-offset:2px; text-decoration:underline; }

/* Intro */
.shopify-pc__prefs__intro-main{ padding:.5rem !important; line-height:1 !important; }
.shopify-pc__prefs__intro-main h3{ font-size:100% !important; }
.shopify-pc__prefs__intro h3{ margin:0 0 6px 0 !important; line-height:1 !important; color:var(--_adult---adult-red) !important; }
...
.shopify-pc__prefs__option{
  text-transform:uppercase !important; border-top:1px solid var(--_adult---adult-red) !important; padding:.5rem !important;
}
.shopify-pc__prefs__option:first-child{ border-top:1px solid var(--_adult---adult-red) !important; padding:.5rem !important; margin:0 !important; }
.shopify-pc__prefs__option label{ color:var(--_adult---adult-red) !important; font-size:100% !important; text-transform:uppercase !important; }
.shopify-pc__prefs__option p{
  color:var(--_adult---adult-red) !important; margin-top:-10px !important; margin-bottom:0px !important; text-transform:none !important;
}

/* Make icon color follow brand red (for our custom SVGs using currentColor) */
.shopify-pc__prefs__option [data-icon-type="checked"],
.shopify-pc__prefs__option [data-icon-type="unchecked"]{
  color:var(--_adult---adult-red) !important; width:24px; height:24px;
}

/* Keep visibility logic consistent */
...
`;
  ...
  // --- swap prefs checkbox icons to Kevin's circular radio-style SVGs ---
  function makeSvg(type){
    ...
    svg.setAttribute('data-icon-type', type);
    // outer circle (stroke uses currentColor)
    var g = document.createElementNS(ns, 'g');
    var outer = document.createElementNS(ns, 'circle');
    outer.setAttribute('cx','12'); outer.setAttribute('cy','12'); outer.setAttribute('r','11.5');
    outer.setAttribute('stroke','currentColor');
    g.appendChild(outer);
    if (type === 'checked'){
      var inner = document.createElementNS(ns, 'circle');
      inner.setAttribute('cx','12'); inner.setAttribute('cy','12'); inner.setAttribute('r','7');
      inner.setAttribute('fill','currentColor'); // brand red via CSS currentColor
      g.appendChild(inner);
    }
    svg.appendChild(g);
    return svg;
  ...
  function swapIcons(root){
    var checkedList = root.querySelectorAll('.shopify-pc__prefs__option [data-icon-type="checked"]');
    var uncheckedList = root.querySelectorAll('.shopify-pc__prefs__option [data-icon-type="unchecked"]');
    checkedList.forEach(function(old){
      var neu = makeSvg('checked');
      old.parentNode && old.parentNode.replaceChild(neu, old);
    });
    uncheckedList.forEach(function(old){
      var neu = makeSvg('unchecked');
      old.parentNode && old.parentNode.replaceChild(neu, old);
    });
  }
  ...
  function inject() {
    ...
    // Unhide banner after styles load
    var h = document.getElementById(HIDE_ID);
    if (h) h.remove();

    // Try immediate swap; then watch for re-renders
    var prefsRoot = document.querySelector('.shopify-pc__prefs__dialog') || document.body;
    swapIcons(prefsRoot);

    // Observe prefs dialog for DOM changes and re-swap if Shopify re-renders
    var obs = new MutationObserver(function(muts){
      muts.forEach(function(m){
        if (m.addedNodes && m.addedNodes.length){
          m.addedNodes.forEach(function(n){
            if (n.nodeType === 1){
              if (n.matches && (n.matches('.shopify-pc__prefs__dialog') || n.closest && n.closest('.shopify-pc__prefs__dialog'))){
                swapIcons(n);
              }
            }
          });

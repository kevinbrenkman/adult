(() => {
  ...
  (async function run(){
    ...
    const tick = (ts) => {
      ...
      // Touch tail (~1s)
      if (Math.abs(touchTailVel) > 1e-4) {
        setProgress(pos + touchTailVel * dt);
        const decay = Math.exp(-dt / TOUCH_TAIL_TAU);
        touchTailVel *= decay;
        if (Math.abs(touchTailVel) < 1e-4) touchTailVel = 0;
      }

(() => {
  ...
  const TOUCH_SPEED       = 0.00018;   // lower sensitivity → slower scroll feel
  const CURSOR_SPEED      = 0.00002;
  const CURSOR_THRESHOLD  = 3;
  const PIN_TO            = '.main-wrapper';

  // Shared momentum (cursor/touch)
  const DAMPING_PER_SEC   = 0.35;   // gentler damping → ~1s longer glide
  const TOUCH_VEL_GAIN    = 0.14;   // slightly stronger flick tail
  ...
  // Wheel kinetic tail
  const WHEEL_TAIL_GAIN   = 2.50;
  const WHEEL_TAIL_TAU    = 2.50;
  ...
  (async function run(){
    ...
    // ===== Core loop (with exponential tail damping)
    const tick = (ts) => {
      ...
      // velocity from touch/cursor
      if (Math.abs(vel) > 1e-4) {
        setProgress(pos + vel * dt);
        // exponential decay — smoother natural damping
